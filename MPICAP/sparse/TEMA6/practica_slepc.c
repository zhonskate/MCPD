...
int main(int argc, char **argv){
  ...
  ierr = MatCreateShell(PETSC_COMM_WORLD, m/size, n/size, m, n, (void*)&A, &Amat);CHKERRQ(ierr);

  MatShellSetOperation(Amat, MATOP_MULT, (void(*)(void))MatMult_ringhals);

  EPS eps;
  ierr = EPSCreate(PETSC_COMM_WORLD,&eps);CHKERRQ(ierr);
  ierr = EPSSetOperators(eps,Amat,NULL);CHKERRQ(ierr);
  ierr = EPSSetProblemType(eps,EPS_NHEP);CHKERRQ(ierr); //problema estandar generalizado
