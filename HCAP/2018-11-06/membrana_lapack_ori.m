...
% esta funci�n visualiza el movimiento de una membrana 
% Condiciones de contorno, inicializaci�n de la soluci�n
xn2=zeros(n+2,n+2);
xn1=zeros(n+2,n+2);
y=zeros(n+2,n+2);
...
incx2=incx*incx;
mu=inct2/incx2;
% Valor inicial de la soluci�n.  
% Aqu� es interesante cambiar uno (o los dos) de los senos por un coseno,   
...
t=0;   
%
% tic, [lu,ind]=dgetrf_mex(A); t=t+toc; % resoluci�n con los mex del ejercicio
% 
for tp=1:100
   % cada cuatro pasos, se genera el gr�fico de la membrana 
   % y se guarda para la pel�cula (un "fotograma").
    if (rem(tp,4)==0)
       surf(y),axis([0 n+2 0 n+2 -0.08 0.08]);
       M(:,tp/4)=getframe;
    end
   % Preparaci�n de la soluci�n 2D en un vector de filas consecutivas
...
y1=matrixDivide64(A,b);
 %y1=dgetrs_mex(lu,b,ind);% resoluci�n con los mex del ejercicio
%  
% %resoluci�n con los archivos .m de la pr�ctica anterior
%    lu=lu_kji(A);
%     yaux=solve_tril_unidad(lu,b);
%     y1=solve_triu(lu,yaux);
%     y1=A\b;
 t=t+toc;  
   % Reconstruimos la soluci�n en 2D para visualizar
