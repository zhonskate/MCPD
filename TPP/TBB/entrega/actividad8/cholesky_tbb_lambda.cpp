...
int main( int argc, char *argv[] ) {
  ...
  */
  ...
  /*********************************************************/
  /* MultiplicaciÃ³n A=L*L', donde L es triangular inferior */
  ...
  */
  ...
  ctimer( &t2, &ucpu, &scpu );
  if( info != 0 ) {
    fprintf(stderr,"Error = %d en la descomposiciÃ³n de Cholesky de la matriz A\n",info);
    exit(-1);
  }
...
int cholesky( int n, int b, double *C ) {
  ...
  for ( k = 0; k < n ; k+=b ) {
    ...
    if( info != 0 ) {
      fprintf(stderr,"Error = %d en la descomposiciÃ³n de Cholesky de la matriz C\n",info);
      return info;
    }
    parallel_for( blocked_range<int>(0,(n-k-1)/b), 
    ...
    parallel_for( blocked_range<int>(0,(n-k-1)/b), 
      ...
      for ( int ii = r.begin(); ii != r.end(); ii++ ) {
        int i = k+b+ii*b;
        int m = min( n-i, b );
        parallel_for( blocked_range<int>(0,(i-k-1)/b), 
          [&]( const blocked_range<int> &r ) {
