...
int cholesky_bloques( int n, int b, double *C ) {
  ...
  #pragma omp single
  for ( k = 0; k < n ; k+=b ) {
    #pragma omp task firstprivate(k) private(m,info) depend(inout:C(k,k))
    {
      m = min( n-k, b );
    ...
    for ( i = k + b; i < n; i += b ) {
      #pragma omp task firstprivate(i,k) private(m) depend(in:C(k,k)) depend(inout:C(i,k))
      {
        m = min( n-i, b );
    ...
    for ( i = k + b; i < n; i += b ) {
        m = min( n-i, b );
        for ( j = k + b; j < i ; j += b ) {
          #pragma omp task firstprivate(i,j,k,m) depend(in:C(i,k),C(j,k)) depend(inout:C(i,j))
          dgemm_( "N", "T", &m, &b, &b, &minusone, &C(i,k), &n, &C(j,k), &n, &one, &C(i,j), &n );
        }
        #pragma omp task firstprivate(i,k,m) depend(in:C(i,k)) depend(inout:C(i,i))
